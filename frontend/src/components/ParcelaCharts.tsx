...
import { Parcela, fetchParcelaHistorico } from '../services/sensorService';
import BoxIcon from './BoxIcon';

interface ParcelaChartsProps {
  parcela: Parcela;
  isOpen: boolean;
  onClose: () => void;
}
...
const ParcelaCharts: React.FC<ParcelaChartsProps> = ({ parcela, isOpen, onClose }) => {
  const [chartType, setChartType] = useState<ChartType>('line');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [historicData, setHistoricData] = useState<Parcela | null>(null);
  ...
  useEffect(() => {
    const loadHistoricData = async () => {
      ...
      setError(null);
      try {
        const data = await fetchParcelaHistorico(parcela.id);
        setHistoricData(data);
      } catch (err) {
        setError('Error al cargar el histórico de la parcela');
      } finally {
        setLoading(false);
      }
    };
  ...
  const filterDataByHour = (data: any[]) => {
  ...
  const getChartData = () => {
    if (!historicData?.lecturas) return [];

    const filteredLecturas = filterDataByHour([...historicData.lecturas].reverse());
    return filteredLecturas.map(lectura => ({
      ...lectura,
      time: formatDateTime(lectura.fecha_lectura),
      lluviaAcumulada: lectura.lluvia
    }));
  };
  ...
  const getChartTitle = () => {
    switch (chartType) {
      case 'line':
        return `Monitoreo de intensidad solar en ${parcela.nombre} durante la última hora`;
      case 'bar':
        return `Registro de temperatura y humedad en ${parcela.nombre} durante la última hora`;
      case 'area':
        return `Acumulación de precipitaciones en ${parcela.nombre} durante la última hora`;
      default:
        return '';
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div ref={modalRef} className="bg-slate-800/90 backdrop-blur-md rounded-xl shadow-xl w-full max-w-6xl max-h-[90vh] overflow-hidden border border-white/10">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-white">
              Gráficos de {parcela.nombre}
            </h2>
            <button
              onClick={onClose}
              className="text-gray-400 hover:text-white transition-colors cursor-pointer"
            >
              <BoxIcon type="regular" name="x" size="md" color='#C3CAD5' />
            </button>
          </div>
